...
> 64MB의 충분한 Write Buffer Size 환경에서는 Write Buffer Number가 성능에 미치는 영향이 제한적이며, 4개 이상에서는 메모리 오버헤드만 증가하고 성능 향상은 없어 수익 체감의 법칙이 적용되었다.


> **원인**: 64MB의 충분한 Write Buffer Size로 인해 Write Stall이 발생하지 않아 추가 버퍼의 필요성이 낮고, 4개 이상에서는 메모리 오버헤드와 캐시 미스 증가로 인한 성능 저하가 발생했기 때문이다.

> - 커지면: 4개까지 성능 향상 → 그 이후 메모리 낭비만 증가하고 성능 정체 (메모리 오버해드, 캐시 미스)
> - 작아지면: 4개가 보다 작아지면 ->  성능 저하 (Flush 대기, CPU 코어 미활용)
